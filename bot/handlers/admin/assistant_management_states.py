...
async def assistant_management_callback(call: CallbackQuery):
    ...
    await safe_edit_message_text(bot, 'Pasirinkite veiksmą:', chat_id=call.message.chat.id,
...
async def assistant_add_callback(call: CallbackQuery):
    ...
    await safe_edit_message_text(bot, 'Siųskite asistento vartotojo vardą:', chat_id=call.message.chat.id,
...
async def assistant_remove_callback(call: CallbackQuery):
    ...
    TgConfig.STATE[f'{user_id}_message_id'] = call.message.message_id
    await safe_edit_message_text(bot, 'Siųskite pašalinamo vartotojo vardą:', chat_id=call.message.chat.id,
                                message_id=call.message.message_id, reply_markup=back('assistant_management'))
...
async def process_assistant_username(message: Message):
    ...
    if not user:
        await safe_edit_message_text(bot, '❌ Vartotojas nerastas', chat_id=message.chat.id,
    ...
    if state == 'assistant_add_username':
        ...
        await safe_edit_message_text(bot, '✅ Asistentas priskirtas', chat_id=message.chat.id,
                                    message_id=message_id, reply_markup=back('assistant_management'))
    else:
        set_role(user.telegram_id, 1)
        await safe_edit_message_text(bot, '✅ Asistentas pašalintas', chat_id=message.chat.id,
                                    message_id=message_id, reply_markup=back('assistant_management'))
